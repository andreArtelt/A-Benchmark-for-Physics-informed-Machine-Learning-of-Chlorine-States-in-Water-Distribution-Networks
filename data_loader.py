...
class DataLoader:
    ...
    def load_unprocessed_data_from_file(self, f_in: str) -> tuple[np.ndarray, np.ndarray, int]:
        ...
        """
        data = np.load(f_in)
        flow_data, chlorine_data, injection_node_idx = data["flow_data"], data["node_quality"], \
            data["injection_node_idx"]

        return flow_data, chlorine_data, int(injection_node_idx)
    
    def load_unprocessed_data_with_topology(self, f_in, f_inp_in, path_in):
        X_flows, X_cl_conc, injection_node_idx = self.load_unprocessed_data_from_file(os.path.join(path_in, f_in))

        with ScenarioSimulator(f_inp_in=f_inp_in) as s:
            topo = s.get_topology()

        return X_flows, X_cl_conc, topo, injection_node_idx
    ...
    def load_unprocessed_data_from_scenarios(self, net_desc: str, random_demands: bool,
        ...
        with Pool(processes=ncpus) as pool:
            ...
            for X_flows_, X_cl_conc_, topo, injection_node_idx in jobs:
                X_flows.append(X_flows_)
                X_cl_conc.append(X_cl_conc_)
                topos.append(topo)

        return np.array(X_flows), np.array(X_cl_conc), injection_node_idx, topos
